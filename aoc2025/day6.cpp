...
constexpr int stoi_impl(std::string_view str, int value = 0) {
  if (str.size() == 0) {
    return value;
  }
  auto first = (str | std::views::take(1))[0];
  auto rest = str | std::views::drop(1);
  return stoi_impl(rest, first - '0' + value * 10);
}
...
constexpr auto clean_input(std::string_view input) {
  ...
  return rows | std::views::transform([not_empty](auto row) {
           return row | std::views::split(' ') | std::views::filter(not_empty) |
                  std::views::transform(
                      [](auto field) { return std::string_view{field}; });
         });
}

constexpr auto get_ops(auto rows) {
  auto row_count = std::ranges::distance(rows);
  return rows | std::views::drop(row_count - 1) | std::views::take(1) |
         std::views::join;
}

constexpr auto get_num_rows(auto cleaned_rows) {
  auto row_count = std::ranges::distance(cleaned_rows);
  return cleaned_rows | std::views::take(row_count - 1) |
         std::views::transform([](auto row) {
           return row |
                  std::views::transform([](auto num) { return stoi(num); });
         });
}

constexpr auto split_by_ops(std::string_view input) {
  auto ops_row = get_ops(clean_input(input));
  return ops_row | std::views::enumerate |
         std::views::transform([input](auto t) {
           auto i = std::get<0>(t);
           auto op = std::get<1>(t);
           auto p = get_num_rows(clean_input(input)) |
                    std::views::transform([i](auto row) {
                      return row | std::views::drop(i) | std::views::take(1);
                    }) |
                    std::views::join;
           return std::tuple{op, p};
         });
}

constexpr auto apply_ops(auto split) {
  size_t applied_ops = 0;

  for (auto [op, nums] : split) {
    if (std::string_view(op).contains('+')) {
      auto sum = std::ranges::fold_left(nums, 0, std::plus<size_t>());
      applied_ops += sum;
    } else if (std::string_view(op).contains('*')) {
      auto prod = std::ranges::fold_left(nums, 1, std::multiplies<size_t>());
      applied_ops += prod;
    }
  }
